...
### Native Voice v0.2.1 — Audio Pipeline Readiness + Finalization Drain (2026-08-22)

**Problem:** Sidney's first real iPhone Vosk Quick Test export (5/7 correct, `falseCardEventCount: 0` — a real, working session) found two bugs. Bug #1: `"Dealer has a five"` (the first phrase spoken) got `audioChunksReceived: 0, partialResultCount: 0, finalizedBy: "never"` — the recognizer received zero audio, not a misrecognition. Bug #2: `"Player three hits."` transcribed as `"player three"` — EyeOnPit safely resolved that to `SELECT_TARGET(seat 3)`, `wouldProduceCardEvent: false` (correct, unchanged), but the trailing word "hits" was lost.

**Bug #1, proven by code inspection:** `voskProvider.ts`'s `onAudioStart` — which the Lab page maps directly to the operator-facing "listening"/SPEAK NOW signal — fired BEFORE requesting microphone permission, constructing the AudioContext, or registering the AudioWorklet processor. A real structural race: the operator could be told to speak while `getUserMedia()`/`audioWorklet.addModule()` (a real async step Mic Check's simpler AnalyserNode-only pipeline never needs — the one concrete architectural difference from Mic Check, which Sidney separately confirmed works correctly on the same device) were still in flight. **Fix:** `onAudioStart` now fires only once a real audio chunk is confirmed delivered by the worklet; a bounded readiness timeout (`VOSK_AUDIO_STARTUP_TIMEOUT_MS`, 5s) surfaces an explicit, disclosed `onError` if zero audio ever arrives, instead of a silent empty phrase. The AudioContext is also now explicitly checked/`resume()`d if not already `"running"` (a real, documented iOS Safari behavior), recorded in the new `audioContextStateAtStart` diagnostic rather than assumed to be the cause.

**Bug #2, a real proven defect (the exact same class already fixed for Sherpa in this codebase's own history — the "SHERPA MIC HARNESS BUG" finalization drain):** `stop()` used to call `recognizer.remove()` and tear down the entire audio pipeline SYNCHRONOUSLY, in the same tick as `retrieveFinalResult()` (which only posts a message — it doesn't wait for a reply). Audio already captured by the AudioWorkletProcessor's own render thread but not yet delivered to the main thread at the exact instant End Phrase was clicked would be silently lost. Whether this specific mechanism (vs. Vosk's own internal endpointer finalizing early — a separate, already-disclosed limitation no drain can fix) is what dropped "hits" in Sidney's session is **not provable** without that phrase's own diagnostics, which weren't captured before this round. **Fix:** `stop()` now drains for `VOSK_AUDIO_DRAIN_MS` (80ms, skipped when a result already arrived spontaneously) before forcing finalization, re-checks for a spontaneous result arriving during the drain, and awaits the forced result's own reply (bounded, `VOSK_FINAL_RESULT_TIMEOUT_MS`) before tearing anything down.

**Hard safety invariant re-confirmed, not just claimed:** "player three" (the real truncated transcript) and "player three hits" (the full phrase) produce the byte-identical `SELECT_TARGET(seat 3)` command — "hits" was always inert filler by design (`parseNarration.ts`'s `INERT_ACTION_WORDS`), so losing it changes nothing about what gets committed. No fuzzy matching, no manufactured action word, added anywhere.

**Diagnostics expanded** (`VoskPhraseDiagnostics`): `micReadyMs`, `audioContextStateAtStart`/`audioContextRunningMs`, `workletReadyMs`, `recognizerReadyMs`, `firstAudioChunkMs`, `nonZeroAudioObserved`/`firstNonZeroAudioChunkMs`, `firstPartialMs`, `endPhrasePressedMs`, `drainDurationMs`, `startupTimedOut` — all elapsed-ms-since-Start-Phrase, exposed in the Lab's Result panel and JSON export. No raw audio retained or uploaded, unchanged.

**Explicitly not touched:** Mic Check (`/lab/mic-check`) — real-device-validated by Sidney this round, frozen, zero diff. `counting-engine/`, Browser Web Speech, Sherpa, Whisper, and 1.10 Split/Double all confirmed untouched. Grammar was not expanded; no multilingual work started.

**Tests:** 32 new (28 provider-level — readiness handshake, drain, repeated-cycle, AudioContext-suspended-state — plus 4 classifier-level safety-invariant regressions). Full suite green (1729 passed, 1 pre-existing skip), `tsc --noEmit` clean, `eslint` clean, production build clean.

**Status:** committed, pushed, deployed. Root causes for BOTH bugs are the most evidence-consistent explanation available from this round's own investigation, disclosed as such — neither is claimed as definitively proven without a real device replay. See this round's own final report for the complete evidence trail and Sidney's next retest procedure.
