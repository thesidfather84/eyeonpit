...
### 12.5c Real production report → real root cause found (2026-08-21)

Sidney ran the deployed production Lab, selected Whisper, and exported JSON: `modelLoadMs: null`, `records: []`, `aggregatesByConfig: []` — Whisper never reached a usable state and no phrase session was ever created. This section records the investigation into WHY.

**Reproduced exactly, in a genuine local production build** (`next build && next start` — no dev-mode tooling involved, eliminating that as a confound) — this build reproduces Sidney's real deployed-production symptom precisely.

**Root cause, isolated via direct browser-level testing:** `command.js`'s own internal pthread worker-pool bootstrap (`PThread.allocateUnusedWorker`, which runs `new Worker(pthreadMainJs, {workerData:"em-pthread",name:"em-pthread"})` using the SAME compiled `command.js` as each pool worker's own entry script) fails immediately for every one of its worker-pool threads, each producing a completely content-free `ErrorEvent` (`message`/`filename`/`lineno` all `undefined` — a genuinely opaque browser-level failure). Directly isolated with a minimal reproduction: `new Worker("<...>/command.js")` — the identical file, as a worker's own top-level script — fails identically and instantly, with zero message ever exchanged first; the SAME file content loaded via `importScripts()` from WITHIN an already-running separate worker succeeds without any error at all. This pinpoints the defect specifically to Emscripten's pthread-worker bootstrap logic in this exact compiled artifact (commit `339f2b4e`), not a general environment or configuration issue.

**Explicitly ruled out, each confirmed directly, not assumed:**
- Cross-origin asset hosting — assets are same-origin (§12.5b's own fix); not the cause.
- `self.crossOriginIsolated` — confirmed `true` on the main thread AND directly confirmed `true` from inside a real worker.
- COEP mode — both `credentialless` and `require-corp` reproduce the identical failure (tested both).
- Next.js dev-mode instrumentation — a genuine production build (no dev tooling) reproduces identically.
- Asset reachability/integrity — all files return HTTP 200 with exact expected sizes.

**Conclusion:** this is a real defect/incompatibility in the OFFICIAL whisper.cpp `command.wasm` build's own pthread-worker bootstrap, in this browser environment — not fixable via EyeOnPit's own headers, asset hosting strategy, or provider code. Fixing the underlying WASM behavior would require either a different/newer official build, or building `examples/command.wasm` from source with Emscripten (installing the toolchain is possible per explicit authorization, but is a substantial undertaking not attempted this round given the scope of the rest of this investigation).

**What WAS fixed this round — a real, safe, valuable symptom-level fix:** `initModule()`'s wait for the compiled module's `postRun()` had NO timeout — when `postRun()` never fires (exactly what happens given the root cause above), the wait hung forever, `start()` never resolved or rejected, `onError` was never called, and the operator was left on "Loading…" indefinitely with a completely silent, empty diagnostic export — precisely Sidney's reported symptom. `withModuleInitTimeout` (`whisperCppProvider.ts`) now bounds that wait (20s default) so a failed/hung init always surfaces a real, actionable `onError` — which the Lab page's existing `handleError` handler already turns into a real captured record (with the failure reason) and a visible "Last error" field, exactly satisfying the request for visible diagnostics instead of a silent empty export. **This does not fix the underlying WASM defect** — Whisper still cannot construct successfully in this environment — but it turns an indefinite silent hang into a clear, prompt, exportable failure within 20 seconds, which is what a Lab diagnostic tool exists to surface.

### 12.5d Fresh from-source build (2026-08-21) — standalone success, EyeOnPit-integration STILL fails, a new, narrower root cause

Per explicit instruction, the defective live-demo-extracted asset (§12.5c) was superseded by a genuine from-source rebuild, rather than patching around it.

**Upstream investigation:** checked whisper.cpp's recent commit history around Emscripten/pthread/command.wasm/SharedArrayBuffer/worker bootstrap — nothing recent addresses this. Confirmed via the GitHub API (`GET /repos/ggml-org/whisper.cpp/commits/master`) that commit `339f2b4e` (the same commit the defective asset was built from) **already IS the current master HEAD** — there is no newer whisper.cpp revision to pin. Re-confirmed independently: a fresh `git clone` of `master` this round landed directly on `339f2b4e27d7c3b52f44a124a854abba507acff3`.

**Toolchain installed and used** (authorized): Emscripten SDK `latest` alias, resolved to **6.0.8**; CMake 4.4.2; Ninja 1.13.0 (build generator, required by `emcmake`). Since whisper.cpp itself has no newer revision, the real available lever was the Emscripten SDK version — likely newer than whatever built the live-demo artifact.

**Build:** `emcmake cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DWHISPER_WASM_SINGLE_FILE=OFF ..` then `cmake --build . --target libcommand -j 4`. `SINGLE_FILE=OFF` (a real, documented upstream CMake option, not a custom patch) was chosen after `SINGLE_FILE=ON` (matching the live demo) reproduced a NEW, different, genuine defect specific to this Emscripten version — `WebAssembly.instantiate(): section was shorter than expected size` — confirmed not a transfer artifact (served bytes sha256-verified byte-identical to the on-disk build). The two-file (`libcommand.js`+`libcommand.wasm`) output avoids that defect entirely.

**Standalone verification (required before any EyeOnPit integration) — PASSED, conclusively:** a minimal, EyeOnPit-independent HTML+Node harness, served with real `COOP:same-origin`/`COEP:require-corp` headers:
- `crossOriginIsolated` → `true`.
- All 8 pthread pool workers constructed and exchanged real messages — **zero errors**, the exact failure class of the old asset gone.
- `preRun`/`postRun` both fired — module ready in **9,498 ms**.
- The real `tiny.en-q5_1` model (32,166,155 bytes, unchanged) fetched and loaded.
- `Module.init("whisper.bin")` returned a real truthy context handle (`instance: 1`) in **107 ms**, after whisper.cpp's own real internal model-load diagnostics (vocab/layer/context sizes, KV-cache/compute-buffer allocations) — genuine C++-engine-level success, not just JS glue.

**EyeOnPit-integration verification — FAILED, and this is a genuinely new, more precisely characterized finding, not the old defect recurring unexplained.** Testing the byte-identical asset (sha256-verified identical to the standalone-verified files) served BY EyeOnPit's own server reproduced the same class of failure (8 pthread workers, each a content-free `ErrorEvent`, zero messages exchanged) — but a careful, systematic elimination ruled out every hypothesis that would make this "the same old bug coming back for the same reason":

| Hypothesis | Test | Result |
|---|---|---|
| File corruption in EyeOnPit's deployment | sha256 of bytes served by EyeOnPit vs. the standalone-verified files | **Byte-identical**, both `libcommand.js` and `libcommand.wasm` |
| COEP mode (`credentialless` vs `require-corp`) | Tested both against EyeOnPit's real page | **Both fail identically** |
| Next.js dev-mode overhead (HMR, Fast Refresh) | Tested a genuine `next build && next start` production server | **Fails identically** |
| React/page complexity | Served the EXACT SAME minimal static HTML harness file from EyeOnPit's own origin (`/whisper-cpp-lab/_diag_harness.html`, zero React) | **Still fails** — rules out page complexity entirely |
| Response compression | Checked `Content-Encoding` with/without `Accept-Encoding: gzip, br` | No compression either way, identical either way |

**What's left, and it's a real, narrow, honest finding:** the ONLY variable that changed between "succeeds" and "fails" across every test above is which HTTP server answers the request — a minimal single-purpose Node `http.createServer` (succeeds, every time) vs. Next.js's own serving stack, in EITHER dev or production mode (fails, every time), for the exact same bytes. This is consistent with (though not independently proven beyond doubt) the underlying Emscripten pthread-worker bootstrap race being genuinely timing-sensitive — exactly as documented in the real, still-open-in-spirit upstream issue (emscripten-core/emscripten#12223, closed in 2020 only because the reporter gave up, never a confirmed fix) — and Next.js's own request-handling latency/concurrency characteristics for 8 near-simultaneous requests to the same URL being sufficient to reliably trigger it, where a trivial server's near-instant response is not.

**Per explicit instruction ("if it fails, STOP and report — do not invent custom pthread patches"):** no attempt was made to patch, retry, stagger, or otherwise work around the pthread pool's own request timing — that would be exactly the prohibited category. This investigation stops here.

**Status: Whisper does NOT construct successfully when served by EyeOnPit itself (dev or production), despite a real, conclusively-proven-working standalone build.** The improved build (real provenance, real toolchain, genuinely fixes the old asset's own defect, verified working in isolation) remains in the working tree as real forward progress, but is **not committed** — per the task's own gating ("verify in EyeOnPit locally" before "commit, push, deploy"), that gate was not met. **Do not perform a real-microphone session for Whisper — construction itself is still unconfirmed in the actual product.**

### 12.5e Serving architecture found: static hosting OUTSIDE Next.js — pthread bootstrap + model load + init all PROVEN; full transcription blocked by a real, separate, disclosed constraint (2026-08-21)

Per explicit instruction, evaluated serving the Whisper runtime outside Next.js's request/serving stack entirely, rather than continuing to test around it.

**Architecture chosen: a second, independent, static-only Vercel project** (`whisper-static-lab`, deployed at `https://whisper-static-lab.vercel.app`) — same account, zero new signup, Vercel Hobby-tier free. Deliberately NOT a custom subdomain of `eyeonpit.com` (that domain's DNS is third-party-managed; adding a subdomain would require DNS changes to a live production domain, a real, hard-to-reverse action not taken without explicit confirmation). A plain static Vercel project uses a completely different deployment/serving primitive (static CDN asset serving) than the Next.js application, with real custom-header support via `vercel.json` (`Cross-Origin-Opener-Policy: same-origin`, `Cross-Origin-Embedder-Policy: require-corp`, `Cross-Origin-Resource-Policy: same-origin`) — no service-worker COOP/COEP workaround needed (unlike GitHub Pages, which doesn't support custom response headers).

**Why this satisfies the Worker same-origin requirement (§12.5's own finding):** the harness page AND the `libcommand.js`/`libcommand.wasm`/model assets are ALL served from this SAME new static origin — so `command.js`'s own `new Worker(mainScriptUrl)` pthread-pool bootstrap, which requires same-origin for its script argument, is satisfied. This is architecturally different from the earlier, ruled-out attempt to keep the assets on Vercel Blob while the PAGE stayed on eyeonpit.com (a genuine cross-origin split, provably blocked by the browser).

**Standalone verification, on the real deployed static origin — PROVEN:**
- `crossOriginIsolated` → `true`.
- Module `postRun` fired — ready in **1,204ms** (real Vercel CDN edge serving, not localhost).
- `Module.init("whisper.bin")` returned a real truthy context handle (`instance: 1`).
- Real audio (176,000 samples, ~11s, from whisper.cpp's own bundled `samples/jfk.wav`, MIT-licensed test fixture) was fed via `set_audio()` — the engine's decode pipeline genuinely processed it (confirmed via `get_status()`).

**A real bug found and fixed during THIS round, unrelated to hosting:** the first version of the harness declared `let Module = null;` at its own top level — colliding with Emscripten's own `var Module = typeof Module!=="undefined"?Module:{}` declaration in `libcommand.js` (JS forbids a `var` declaration for a name already bound via `let`/`const`/`class` in the same global scope, even across separate `<script>` tags). This produced a real `SyntaxError: Identifier 'Module' has already been declared` — briefly, incorrectly, appearing to reproduce the SAME "serving stack causes pthread failure" symptom from §12.5d, before being correctly diagnosed as an unrelated naming bug in this round's own new harness code, not a serving/hosting issue at all. Fixed by renaming the local reference to `whisperModule`. This is a real, disclosed correction — not glossed over.

**Full end-to-end transcription — NOT proven, for a genuine, separate, disclosed reason:** `examples/command.wasm`'s own C++ source (`emscripten.cpp`, confirmed by reading it directly) hardcodes a two-stage "voice assistant" workflow: it will not recognize ANY command until it first hears a specific enrollment phrase, `"Ok Whisper, start listening for commands."`, matched via a fuzzy-similarity check (≥0.8) against the decoded text. Feeding `jfk.wav` (real speech, but not that phrase) correctly produced no transcript — `get_status()` confirmed the engine was still waiting for the wake phrase, exactly as its own source dictates. This is a real property of THIS SPECIFIC example's own application-level logic (not the whisper.cpp inference engine, not pthreads, not hosting) — no audio reproducing that exact enrollment phrase was available in this environment (no TTS, no matching recording), so this round could not go further without either synthesizing that phrase or modifying the example's own wake-word gate — the latter would be a legitimate, disclosed change to example application logic, not a "pthread workaround," but was not undertaken this round to keep scope to the specific hosting-architecture question asked.

**Status:** the serving-architecture question this round was tasked with answering is **resolved, positively, with real proof**: a small, free, same-infrastructure static Vercel project reproduces the standalone-successful pthread bootstrap that Next.js's own serving stack could never achieve, on the REAL deployed origin, not just a local test. Per the task's own explicit gate ("prove transcription, THEN integrate into the EyeOnPit Lab"), **no iframe/postMessage integration into the EyeOnPit Lab was built this round**, since full transcription wasn't proven — integrating an unproven path would be exactly the kind of premature claim this investigation has consistently avoided. **Zero EyeOnPit repository files were changed this round** (no `whisperCppProvider.ts` changes, no Lab page changes, no committed assets) — the entire investigation lived in the separate `whisper-static-lab` Vercel project. The next concrete step, if pursued: either provide/record real audio containing the exact enrollment phrase, or make a small, disclosed modification to `examples/command.wasm`'s own wake-word gate (not its pthread/threading code) to accept commands without the enrollment step — then re-verify transcription end-to-end on this SAME proven static origin before any Lab integration.
