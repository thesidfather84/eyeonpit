...
| Voice: contextual dealer-confusion recovery ("Taylor"/"Spotify" -> DEALER, ORDERED multi-card hands, last-resort only, narrow closed grammar) | **IMPLEMENTED** | `tryDealerConfusionRecovery` in [`lib/voice/classifyVoiceTranscript.ts`](../src/lib/voice/classifyVoiceTranscript.ts), invoked only from nBestResolver's zero-valid-alternatives fallback in [`lib/voice/nBestResolver.ts`](../src/lib/voice/nBestResolver.ts); committed via `commitNarration` in [`components/live/VoiceControl.tsx`](../src/components/live/VoiceControl.tsx) |
| Voice: target-omitted continuation ("has a ten and a three" against a live active target, gated on genuine connector-word evidence) | **IMPLEMENTED** | `allowUnscopedContinuation`/`sawUnscopedConnector` in [`lib/voice/parseNarration.ts`](../src/lib/voice/parseNarration.ts) — see `docs/EYEONPIT_VOICE_FIELD_TEST_2.md` §5 |
| Voice: natural target-setting intent ("current player is spot one" / "watching spot one", no CardEvent) | **IMPLEMENTED** | [`lib/voice/parseSetActiveTargetIntent.ts`](../src/lib/voice/parseSetActiveTargetIntent.ts) |
| Voice: numeric/time-pattern safety (a colon/slash clock-time or fraction shape never decomposes into cards, even against an active target) | **IMPLEMENTED** | `containsTimeOrFractionPattern` in [`lib/voice/parseVoiceCommand.ts`](../src/lib/voice/parseVoiceCommand.ts) |
| Voice: session reliability metrics (acceptance rate, ASR_NO_FINAL rate, rescue counts, timing) | **IMPLEMENTED** | [`lib/voice/sessionMetrics.ts`](../src/lib/voice/sessionMetrics.ts), surfaced in [`components/live/VoiceDiagnosticsPanel.tsx`](../src/components/live/VoiceDiagnosticsPanel.tsx) |
